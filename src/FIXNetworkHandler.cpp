...
void FIXNetworkHandler::Test(const char* test_type, const int client_socket_fd){
    char in_buffer[1024] = {};
    const char* out_buffer{"PONG"};
    const char* exit_signal{"EXIT"};
    // initiate latency test
    int nbytes_s{send(client_socket_fd, test_type, strlen(test_type), 
                   0)};

    
    int nbytes_r{};
    while((nbytes_r = recv(client_socket_fd, in_buffer, 
