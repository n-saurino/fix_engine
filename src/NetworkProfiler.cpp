...
void NetworkProfiler::Benchmark(int client_socket_fd){
    ...
    int nbytes_r{};
    while((nbytes_r = 
          recv(client_socket_fd, in_buffer, sizeof(in_buffer) - 1, 0)) > 0){
        if(nbytes_r > 0){
...
void NetworkProfiler::BenchmarkLatency(int client_socket_fd){
    ...
    std::cout << "Average client round-trip-time: " 
              << total_latency / num_events << " nanoseconds\n"; 
...
void NetworkProfiler::BenchmarkThroughput(int client_socket_fd){
    ...
    std::cout << "Average client throughput: "
              << total_messages / duration << " round-trip-transmissions\n";
